...
# types of comment assignment
# Up until, including, 0.17.x comments are parsed and following empty- and comment-only-
# lines collated, then attached to the collection node being parsed. And not
# to any following nodes.
# The elements attached to the original Comment instances are CommentTokens that
# are re-inserted on the Nodes generated by the Representer and then written when
# found by the Emitter.
# This works well when you only change values in the datastructure, but not if you add
# things to that structure.
# Starting with 0.17 you can postprocess the comments, this involves walking
# over the datastructure recursively, splitting any multiline comments (i.e. the strings)
# resulting for any previously collated follow up comment lines, in an offset relative to the
# collection. Assignment to which key/element lines belong is determined by a bitset, allowing
# for selection of preceding or following, split on empty line.
# At the beginning or end of a collection (but not in the middle), negative
# indentation relative to the collection cause the comment to be attached to the
# appropriate parent collection.
# ToDo: Empty lines, are these like non-indented comments? Attach like preceding comment?
...
######################################################################
# combine all extra attributes in one
class YamlData:
    __slots__ = '_comment'

    def __init__(self) -> None:
        self._comment: Union[Comment, None] = None

    @property
    def comment(self) -> Union[Comment, None]:
...
class CommentedBase:
    @property
    def rya(self) -> Any:
        if not hasattr(self, RYA):
            setattr(self, RYA, YamlData())
        return getattr(self, RYA)

    @property
    def ca(self) -> Any:
        if self.rya.comment is None:
            self.rya.comment = Comment()
        return self.rya.comment
    ...
    def copy_attributes(self, t: Any, memo: Any = None) -> None:
        # fmt: off
        if memo is not None:
            t.rya.comment = copy.deepcopy(self.rya.comment)
        else:
            t.rya.comment = self.rya.comment
        for a in [Format.attrib, LineCol.attrib, Anchor.attrib,
